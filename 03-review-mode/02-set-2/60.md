...
**DDoS protection using CloudFront + AWS WAF.**

### ✅ Correct answer

**CloudFront → ALB origin + AWS WAF rate-based rule.**

* **CloudFront** absorbs/distributes large amounts of traffic at edge locations.
* **AWS WAF** can detect/block excessive requests using a **rate-based rule**.
* WAF can be associated directly with CloudFront.

🔥 **Exam shortcut:**
**DDoS + web application → CloudFront + WAF**

❌ GuardDuty/VPC Flow Logs → detection/monitoring, not the most effective immediate DDoS protection.
❌ Security Group → not designed for application-layer DDoS/rate limiting.
